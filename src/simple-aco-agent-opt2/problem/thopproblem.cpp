...
void compute_nn_lists() {
    int n = nCities;
    nn_list.assign(n, vector<int>());
    int k = min((int)params.nn_ants, n - 1);
    
    for (int i = 0; i < n; ++i) {
        vector<pair<long double, int>> dists;
        for (int j = 0; j < n; ++j) {
            if (i != j) {
                dists.push_back({distance(i, j), j});
            }
        }
        sort(dists.begin(), dists.end());
        for (int j = 0; j < k; ++j) {
            nn_list[i].push_back(dists[j].second);
        }
    }
    log_debug("thopproblem: computed nearest neighbor lists (nn_ants = ", k, ")");
}
...
bool readData(const string& path) {
    ...
    is_ceil_2d = (edgeWeightType == "CEIL_2D" || edgeWeightType.find("CEIL_2D") != string::npos);
