 ...
 */
{
    ...
    if (!log_flag) {
        ...
        /* Redirect stderr into <output>.log so ALL fprintf(stderr, ...)
           (e.g. PACKLOG in thop.c) land in the log file, not the terminal. */
        log_file = freopen(temp_buffer, "w", stderr);
    } else {
        log_file = NULL;
    }
 ...
 */
{
    ...
    if ( mmas_flag ) {
        ...
        fprintf(stderr, "PHEROMONE|trail_0=%.15e\n", trail_max);
